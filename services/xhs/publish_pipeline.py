...
将所有发布步骤串联成一条完整 pipeline：
  启动 Chrome → 检查登录 → 下载图片/视频 → 填写表单 → 输入标签 → 点击发布

本脚本可以独立于 Slack Bot 使用（比如定时任务、批量发布等场景）。

用法示例：
    # 图片 URL 模式 — 下载远程图片后发布（默认立即发布）
...
退出码：
    0 = 成功（PUBLISHED 或预览模式的 READY_TO_PUBLISH）
    1 = 未登录（NOT_LOGGED_IN）— headless 模式下会自动切换到有界面模式
    2 = 出错（详见 stderr）
    3 = 锁冲突（另一个发布任务正在运行）
"""

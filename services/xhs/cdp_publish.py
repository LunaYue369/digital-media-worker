...
class XiaohongshuPublisher:
    ...
    def _scroll_element_into_view(self, js_find_element: str):
        """将指定元素滚动到视口中央

        Args:
            js_find_element: JavaScript 表达式，返回目标 DOM 元素或 null
        """
        js_scroll = f"""
            (function() {{
                var el = {js_find_element};
                if (el) {{
        ...
        """
        self._evaluate(js_scroll)
        time.sleep(0.3)

    def _click_element_by_cdp(self, description: str, js_get_rect: str,
                               js_find_element: str = None):
        """通过 CDP 鼠标事件点击元素（比 JS .click() 更可靠）

        现代前端框架（Vue/React）经常忽略 JS .click() 调用，
        通过 CDP 分发真实鼠标事件则始终有效。

        流程：
        1. 如果提供了 js_find_element，先将元素滚动到视口可见区域
        2. 执行 js_get_rect 获取元素视口坐标
        3. 通过 CDP Input.dispatchMouseEvent 分发鼠标按下/释放事件

        Args:
            description: 元素描述（用于日志输出）
            js_get_rect: JavaScript 表达式，返回 {x, y, width, height} 或 null
            js_find_element: JavaScript 表达式，返回目标 DOM 元素（用于 scrollIntoView）。
                             如果为 None 则跳过滚动，直接点击。
        """
        # 先将元素滚动到视口可见区域（如果提供了定位表达式）
        if js_find_element:
            self._scroll_element_into_view(js_find_element)
    ...
    def _click_publish(self, scheduled: bool = False):
        ...
        # 定位发布按钮 DOM 元素的 JS（用于 scrollIntoView）
        js_find_btn = f"""
            (function() {{
                var buttons = document.querySelectorAll('button');
                for (var i = 0; i < buttons.length; i++) {{
                    if (buttons[i].textContent.trim() === '{btn_text}') return buttons[i];
                }}
                var spans = document.querySelectorAll('.d-button-content .d-text, .d-button-content span');
                for (var i = 0; i < spans.length; i++) {{
                    if (spans[i].textContent.trim() === '{btn_text}') {{
                        return spans[i].closest('button, [role="button"], .d-button, [class*="btn"], [class*="button"]') || spans[i];
                    }}
                }}
                return null;
            }})()
        ...
        # 先尝试 CDP 鼠标事件点击（更接近真实用户行为）
        try:
            self._click_element_by_cdp("publish button", js_get_rect, js_find_element=js_find_btn)
        except CDPError:
            pass

        # 等待一小段时间，检查是否成功（页面是否离开了发布页）
        self._sleep(2, minimum_seconds=1.0)
        current_url = self._evaluate("window.location.href")
        if "publish/publish" in current_url:
            # CDP 鼠标事件没生效，回退到 JS .click()
            print("[cdp_publish] CDP click may not have worked, trying JS click fallback...")
            js_click = f"""
                (function() {{
                    var buttons = document.querySelectorAll('button');
                    for (var i = 0; i < buttons.length; i++) {{
            ...
            """
            result = self._evaluate(js_click)
            if result == "not_found":
                raise CDPError("无法找到发布按钮，请在浏览器中手动点击。")
            print("[cdp_publish] JS click fallback executed.")

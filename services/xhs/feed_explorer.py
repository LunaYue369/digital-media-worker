...
从小红书页面的 window.__INITIAL_STATE__ 中提取数据，包括：
- 搜索笔记列表（按关键词搜索，支持筛选排序）
- 获取单篇笔记详情

本模块不直接操控浏览器，而是提供可复用的数据提取逻辑，
由 cdp_publish.py 的 XiaohongshuPublisher 注入 evaluate/sleep/mouse 等回调来驱动。
"""
